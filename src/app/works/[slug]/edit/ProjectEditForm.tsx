    ...
    async function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
        ...
        try {
            ...
            if (!res.ok) {
                ...
                const detailStr = details ? Object.entries(details).map(([k, v]: [string, any]) => `· ${k}: ${Array.isArray(v) ? v.join(", ") : v}`).join("\n") : "";
                throw new Error(detailStr ? `${errMsg}\n${detailStr}` : errMsg);
            }
            setSaved(true);
            router.refresh();
        } catch (err: any) {
        ...
        return (
            <div className="animate-fade-in text-center py-16">
                ...
                <a
                    ...
                    onClick={(e) => {
                        e.preventDefault();
                        router.push(`/works/${initialData.slug}`);
                    }}
                >
                    返回作品页
                </a>
        ...
        <form onSubmit={handleSubmit} className="space-y-8 animate-fade-in">
            ...
            <section className="space-y-4">
                ...
                <div>
                    ...
                    <input name="title" type="text" required maxLength={200} defaultValue={initialData.title} className={inputClass}
                           style={inputStyle}/>
                </div>

                <div>
                    <label className={labelClass} style={labelStyle}>副标题</label>
                    <input name="subtitle" type="text" maxLength={300} defaultValue={initialData.subtitle} className={inputClass}
                           style={inputStyle}/>
                </div>

                <div>
                    <label className={labelClass} style={labelStyle}>简介 <span style={{color: "#C62828"}}>*</span></label>
                    <textarea name="description" required minLength={10} maxLength={10000} rows={4} defaultValue={initialData.description}
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    ...
                    <div>
                        ...
                        <input name="developYear" type="number" required min={2000} max={new Date().getFullYear() + 1}
                               defaultValue={initialData.developYear} className={inputClass} style={inputStyle}/>
                    </div>
                </div>
            ...
            <section className="space-y-3">
                ...
                {showCoverCrop && coverCropSrc && (
                    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60"
                         onClick={() => setShowCoverCrop(false)}>
                        <div className="bg-white rounded-xl p-6 w-[90vw] max-w-lg shadow-2xl" onClick={(e) => e.stopPropagation()}>
                            ...
                            <div className="flex justify-end gap-3 mt-4">
                                <button type="button" className="px-4 py-2 rounded-lg text-sm"
                                        style={{color: "#777", border: "1px solid #D0DEE8"}}
                                        onClick={() => {
                                        ...
                                        onClick={async () => {
                                            ...
                                            try {
                                                const {
                                                    file: croppedFile,
                                                    url: previewUrl
                                                } = await getCroppedImg(coverCropSrc, coverCropped);
                ...
                <div className="flex items-start gap-4">
                    <div className="shrink-0">
                        {coverPreview ? (
                            <img src={coverPreview} alt="封面预览" className="w-40 h-24 object-cover rounded-lg border"
            ...
            <section className="space-y-3">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-[1fr_160px] gap-2" ref={memberDropdownRef}>
                    <div className="relative">
                        <input type="text" value={memberQuery} onChange={(e) => {
                            setMemberQuery(e.target.value);
                            setShowMemberDropdown(true);
                        }} onFocus={() => setShowMemberDropdown(true)} placeholder="搜索社团成员姓名..." className={inputClass}
                               style={inputStyle}/>
                            ...
                            <div className="absolute z-20 mt-1 w-full bg-white border rounded-lg shadow-lg max-h-48 overflow-y-auto"
                                ...
                                {memberResults.map((m: any) => (
                                    <button key={m.id} type="button" onClick={() => addMember(m)}
                                        ...
                                        <span
                                            className="w-6 h-6 rounded-full bg-[#E38043] text-white text-xs flex items-center justify-center flex-shrink-0">{m.displayName.charAt(0)}</span>
                                        <span style={{color: "#333"}}>{m.displayName}</span>
                                        <span className="text-xs ml-auto"
                                              style={{color: "#999"}}>{m.grade && `${m.grade} · `}</span>
                                    </button>
                                ))}
                            </div>
                        )}
                    </div>
                    <input type="text" value={memberRole} onChange={(e) => setMemberRole(e.target.value)} onKeyDown={(e) => {
                        if (e.key === "Enter") {
                            e.preventDefault();
                        }
                    ...
                    <div className="flex flex-wrap gap-2">
                        {selectedMembers.map((m, i) => (
                            <span key={i} className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs"
                                  style={m.memberId ? {
                                      background: "rgba(37,84,122,0.1)",
                                      color: "#25547A"
                                  } : {background: "rgba(227,128,67,0.1)", color: "#E38043"}}>
                {m.memberId && <span
                    className="w-4 h-4 rounded-full bg-[#E38043] text-white text-[10px] flex items-center justify-center">{m.displayName.charAt(0)}</span>}
            ...
            <section className="space-y-3">
                        ...
                        {links.map((link, i) => (
                            <div key={i} className="flex items-center gap-2 px-3 py-2 bg-[#F0F5F9] rounded-lg text-sm">
                                <span className="px-2 py-0.5 rounded text-xs font-medium flex-shrink-0"
                                      style={{background: "#25547A", color: "#fff"}}>{link.label}</span>
                                <span className="truncate flex-1" style={{color: "#3388BB"}}>{link.url}</span>
                                <button type="button" onClick={() => removeLink(i)} className="text-xs flex-shrink-0 hover:text-red-500"
                                        style={{color: "#999"}}>移除
                                </button>
                            </div>
                        ))}
                ...
                <div className="flex flex-wrap items-end gap-2">
                    {!showCustomLabel ? (
                        ...
                        <input type="text" value={newLinkLabel} onChange={(e) => setNewLinkLabel(e.target.value)} onKeyDown={(e) => {
                            if (e.key === "Enter") {
                                e.preventDefault();
                                addLink();
                            }
                    ...
                    <input type="url" value={newLinkUrl} onChange={(e) => setNewLinkUrl(e.target.value)} onKeyDown={(e) => {
                        if (e.key === "Enter") {
                            e.preventDefault();
                            addLink();
                        }
                    ...
                    <button type="button" onClick={addLink} disabled={!newLinkLabel.trim() || !newLinkUrl.trim()}
                            ...
                            style={{background: "#88C232", color: "#fff"}}>添加
                    </button>
                </div>
            </section>
            ...
            <section className="space-y-3">
                        ...
                        {tags.map((tag) => {
                            ...
                            return (
                                <button key={tag.id} type="button" onClick={() => toggleTag(tag.id)}
                                        ...
                                        style={{
                                            backgroundColor: active ? "rgba(136,194,50,0.15)" : "rgba(136,194,50,0.08)",
                                            color: "#88C232"
                                        }}>
                                    {tag.name}
                                </button>
                ...
                <div className="flex gap-2">
                    <input type="text" value={customTagInput} onChange={(e) => setCustomTagInput(e.target.value)} onKeyDown={(e) => {
                        if (e.key === "Enter") {
                            e.preventDefault();
                            addCustomTag();
                        }
                    ...
                    <button type="button" onClick={addCustomTag} disabled={!customTagInput.trim()}
                            ...
                            style={{background: "#E38043", color: "#fff"}}>添加
                    </button>
                </div>
            </section>
