...
interface Project {
  id: string; slug: string; title: string; subtitle: string | null; coverImage: string | null; type: string;
  tags?: { tag: { name: string } }[];
  members?: { memberId: string | null; externalName: string | null; roles: string[]; member?: { displayName: string } | null }[];
  _count?: { likes: number };
}
interface EventItem {
  id: string; title: string; body: string | null; eventDate: string | null;
...
interface Props {
  year: number; members: Member[]; projects: Project[]; events: EventItem[];
  activities: Activity[]; activeMembers: ActiveMember[]; startYear: number;
}
...
function generateLead(year: number, members: Member[], projects: Project[], activities: Activity[], events: EventItem[], yearIndex: number, activeMembers: ActiveMember[]): string {
  ...
  if (yearIndex === 1) {
    ...
    lines.push(`${year}年，布谷工作室走过了第 ${yearIndex} 个年头，步履愈发坚定。`);
  } else {
    lines.push(`${year}年是布谷工作室的第 ${yearIndex} 个年头。从初创时的青涩到如今的稳步前行，每一届成员都在这里留下了独特的印记。`);
  }
  ...
  if (activeMembers.length > 0) {
    lines.push(`全年共有 ${activeMembers.length} 位成员活跃在创作一线，他们的身影出现在作品、比赛、公开课和例会分享的各个角落。`);
  }
  ...
  if (projects.length > 0) {
    ...
    lines.push(`全年共产出 ${projects.length} 件作品（${parts.join("、")}），每一件都凝结着团队的心血与创意。`);
  }
  ...
  if (activities.length > 0) {
    ...
    const parts: string[] = [];
    for (const t of ACT_ORDER) { if (actCounts[t]) parts.push(`${ACT_LABELS[t]} ${actCounts[t]} 场`); }
    lines.push(`全年举办活动 ${activities.length} 场（${parts.join("、")}），展现了团队的活力与凝聚力。`);
  }
...
//  颜色常量
// ═══════════════════════════════════════════════════════
const A = "#25547A"; const B = "#E38043"; const C = "#88C232";
const PAPER = "#faf8f5"; const INK = "#2c1810"; const INK2 = "#4a3728"; const INK3 = "#6b5d4f";
const LINE = "#d4c5b2"; const LINEL = "#e8ddd0"; const GOLD = "#c4a86a";
...
const Q: Record<string, React.CSSProperties> = {
  ...
  paper: { background: PAPER, border: `1px solid ${LINE}`, boxShadow: "0 2px 20px rgba(100,80,60,0.07)", fontFamily: "'Noto Serif SC','STSong','SimSun',Georgia,serif", color: INK },
  // 报头
  masthead: { textAlign: "center" as const, padding: "22px 40px 14px" },
  mastTop: { height: 3, background: `linear-gradient(90deg,transparent 8%,${GOLD} 20%,${A} 35%,${A} 65%,${GOLD} 80%,transparent 92%)`, marginBottom: 16 },
  mastBot: { height: 3, background: `linear-gradient(90deg,transparent 8%,${GOLD} 20%,${A} 35%,${A} 65%,${GOLD} 80%,transparent 92%)`, marginTop: 16 },
  ...
  dropCap: { fontSize: 42, fontWeight: 900, color: A, lineHeight: 0.8, marginRight: 6, fontFamily: "Georgia,serif", flexShrink: 0, marginTop: -2 },
  leadText: { fontSize: 14, color: INK2, textAlign: "justify" as const, letterSpacing: 0.3 },
  // 分隔线
  rule: { height: 1, margin: "14px 0 18px", background: `linear-gradient(90deg,transparent 15%,${LINEL} 35%,${GOLD} 50%,${LINEL} 65%,transparent 85%)` },
  thickRule: { height: 2, margin: "20px 0", background: `linear-gradient(90deg,transparent 10%,${LINE} 30%,${GOLD} 50%,${LINE} 70%,transparent 90%)` },
  // 双栏数字面板
  twoCol: { display: "grid", gridTemplateColumns: "1fr 1fr", gap: 24, marginBottom: 6 },
  statPanel: { padding: "14px 20px", background: "linear-gradient(135deg,rgba(255,255,255,0.5),rgba(232,221,208,0.2))", border: `1px solid ${LINEL}` },
  panelLabel: { fontSize: 11, color: GOLD, letterSpacing: 6, marginBottom: 12, textAlign: "center" as const, fontFamily: "system-ui,'Microsoft YaHei',sans-serif" },
  ...
  // 章节标题
  sectionTitle: { fontSize: 15, fontWeight: 700, color: GOLD, letterSpacing: 8, textAlign: "center" as const, fontFamily: "system-ui,'Microsoft YaHei',sans-serif", marginBottom: 16 },
  // ─── 作品巡礼：代表作 ───
  ...
  featuredImgPH: { width: "100%", height: "100%", display: "flex", alignItems: "center", justifyContent: "center", background: `linear-gradient(135deg,${A},#4488aa)` },
  ...
  featuredTypeTag: { fontSize: 11, color: "#fff", background: GOLD, padding: "3px 10px", fontFamily: "system-ui,sans-serif", fontWeight: 600, letterSpacing: 2 },
  ...
  workImgPH: { width: "100%", height: "100%", display: "flex", alignItems: "center", justifyContent: "center", background: `linear-gradient(135deg,${A},#4488aa)` },
  workPHIcon: { fontSize: 24, opacity: 0.4 },
  workCaption: { display: "flex", flexDirection: "column" as const, gap: 2 },
  workName: { fontSize: 13, fontWeight: 700, color: INK, fontFamily: "system-ui,'Microsoft YaHei',sans-serif", display: "flex", alignItems: "baseline", gap: 6 },
  ...
  rosterAvatar: { width: 34, height: 34, borderRadius: "50%", overflow: "hidden", background: `linear-gradient(135deg,${A},#4477aa)`, display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 },
  ...
  highlightItem: { fontSize: 13, lineHeight: 1.9, color: INK2, margin: 0, display: "flex", alignItems: "flex-start", gap: 12, textAlign: "justify" as const },
  // ─── 活动回顾 ───
  ...
  tlMarker: { display: "flex", flexDirection: "column" as const, alignItems: "center", width: 14, flexShrink: 0, position: "relative" as const },
  tlDot: { width: 10, height: 10, borderRadius: "50%", background: C, marginTop: 8, flexShrink: 0, position: "relative" as const, zIndex: 1 },
...
function saveBtnStyle(loading: boolean): React.CSSProperties {
  return { display: "inline-flex", alignItems: "center", gap: 6, padding: "8px 18px", borderRadius: 8, border: `1.5px solid ${A}`, background: loading ? A : "transparent", color: loading ? "#fff" : A, fontSize: 13, fontWeight: 600, cursor: loading ? "default" : "pointer", fontFamily: "system-ui,sans-serif", transition: "all 0.2s" };
}
...
function highlightDropStyle(i: number): React.CSSProperties {
  ...
  return { width: sizes[i] || 34, height: sizes[i] || 34, borderRadius: "50%", flexShrink: 0, background: `linear-gradient(135deg,${colors[i] || "#a0a0a0"},${i === 0 ? "#d4b87a" : i === 1 ? "#c0c0c0" : "#d4a57a"})`, display: "flex", alignItems: "center", justifyContent: "center", color: "#fff", fontSize: 15, fontWeight: 700, fontFamily: "system-ui,sans-serif", overflow: "hidden" };
}
function activityTypeTagStyle(t: string): React.CSSProperties {
  const colors: Record<string, string> = { COMPETITION: B, COURSE: A, GENERAL: C, MEETING: LINE };
  return { fontSize: 10, fontWeight: 600, color: "#fff", background: colors[t] || LINE, padding: "2px 8px", fontFamily: "system-ui,sans-serif", letterSpacing: 1, flexShrink: 0 };
}
...
function StatBox({ num, label }: { num: number; label: string }) {
  return <div style={STAT_Q.statBox}><div style={STAT_Q.statNum}>{num}</div><div style={STAT_Q.statLabel}>{label}</div></div>;
}
            ...
            <>
              ...
              {meetingCount > 0 && (
                <p style={{ fontSize: 13, color: INK3, lineHeight: 2, textAlign: "justify" as const, marginBottom: 16 }}>
                  全年共举办了 {meetingCount} 场例会，定期碰面、同步进度、头脑风暴，确保团队的方向始终一致。
                </p>
              )}
